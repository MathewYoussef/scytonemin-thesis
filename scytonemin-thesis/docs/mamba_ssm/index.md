...
The Act-of-God (AoG) run promotes the best-performing fold-02 checkpoint into an all-data model, safeguarded by a fixed validation panel. In this repo the full evaluation kit ships pre-packaged: raw spectra under `data/raw/mamba_ssm/`, manifests and metadata in `data/reference/mamba_ssm/`, checkpoints in `models/mamba_ssm/`, and evaluation scripts under `src/mamba_ssm/`.

AoG denoising is **not** re-run automatically. We ship the original checkpoints and panel metrics; evaluators may optionally install the heavy dependencies (`torch`, `torchvision`, `mamba-ssm[causal-conv1d]`) and execute `python -m mamba_ssm.scripts.evaluate_validation_panel` to reproduce the numbers.
...
- **Notebooks:** `notebooks/mamba_validation_overview.ipynb` previews the validation panel aggregation; executed evaluations should land in `scaffold/mamba_ssm/notebooks/`.
...
- Confirm checkpoint and metrics manifests (`models/mamba_ssm/CHECKSUMS.sha256`, `ops/output/data/mamba_checks/CHECKSUMS.sha256`) when new artefacts arrive.
- Document any optional reruns (panel eval, downstream proxy) in `ops/logs/` so auditors understand whether Torch-based tools were invoked.
