...
This block documents the chromatogram → DAD → concentration → dry weight pipeline that underpins the thesis calibration narrative. All required inputs now live inside `scytonemin-thesis/`: raw instrument exports in `data/raw/initial_calibration/`, canonical outputs/configs in `data/reference/initial_calibration/`, and executable scripts under `src/chromatography/`.

Run the pipeline locally with:

```bash
make setup        # install dependencies
make reproduce    # executes Stage A (calibration) and Stage B/C (treatment quantification)
```

These commands regenerate the canonical tables in `data/reference/initial_calibration/` and refresh scaffold figures.
...
- **Notebooks:** `notebooks/initial_calibration_overview.ipynb` for the sample walkthrough; executed outputs belong in `scaffold/initial_calibration/notebooks/`.
...
- Confirm checksum manifests under `data/raw/initial_calibration/` and `data/reference/initial_calibration/` after each regeneration run.
- Extend regression coverage (Stage A/B assertions) in `tests/chromatography/` once the shared test suite lands.
