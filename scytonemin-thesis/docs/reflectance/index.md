...
This block curates the denoising, spectral analysis, and downstream comparisons for the field reflectance campaign. The entire toolchain is bundled in `scytonemin-thesis/`: raw and staging spectra in `data/raw/reflectance/`, canonical exports in `data/reference/reflectance/`, and harmonisation scripts under `src/reflectance/`.

Rebuild the canonical bundle with:

```bash
make reproduce        # runs Stage A/B and reflectance canonical builder
```

The canonical tables land in `data/reference/reflectance/canonical_dataset/`, and scaffold figures update automatically from the packaged AoG outputs.
...
- **Notebooks:** `notebooks/03_spectra_ingest_and_rel_reflectance.ipynb` (raw counts → relative reflectance) and `notebooks/reflectance_overview.ipynb` (canonical dataset tour); executed runs should land in `scaffold/reflectance/notebooks/`.
...
- Verify checksum manifests in `data/raw/reflectance/` and `data/reference/reflectance/` after each canonical build.
- Add smoke/regression tests in `tests/reflectance/` (e.g., canonical dataset invariants) once Agents 3/4 contribute analysis notebooks.
