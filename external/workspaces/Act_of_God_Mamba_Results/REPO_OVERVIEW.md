...
| Path / File | Purpose |
| ----------- | ------- |
| `README.md` | Warm-start plan; high-level objectives, validation gates, training command. |
| `REPO_OVERVIEW.md` | *This* document – directory tour + usage guide. |
...
- `panel_eval_summary.json` – machine-readable QA results (per-gate pass/fail, manifest hash, git hash, ranked checkpoints).
- `README_audit.md` – human summary of the QA outcome (aggregate fail, ROI fail, downstream pass).
...
3. Run:
   ...
   python -m src.train \
     ...
     ...  # see README.md “Training recipe” for full argument list
   ```
