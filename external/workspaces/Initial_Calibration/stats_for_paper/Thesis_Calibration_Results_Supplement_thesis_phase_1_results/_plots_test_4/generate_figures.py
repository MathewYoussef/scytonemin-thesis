...
def figure04_single_factor_dad() -> None:
    """DAD total amount vs dose metrics (mg·gDW⁻¹)."""
    ...
    for ax, (label, x_col) in zip(axes, specs):
        sns.regplot(
            x=x_col,
            y="predicted_total_mg_per_gDW",
            data=df,
            ax=ax,
            ci=95,
            scatter_kws=dict(s=70, alpha=0.85),
        ...
        model = sm.OLS(df["predicted_total_mg_per_gDW"], sm.add_constant(df[x_col])).fit()
        slope = model.params[x_col]
        slope_se = model.bse[x_col]
        r2 = model.rsquared
    ...
    fig.suptitle(
        "Fig. 4 — DAD single-factor trends (mg·gDW$^{-1}$)",
        fontsize=18,
    )
...
def figure05_emms() -> None:
    ...
    fig.suptitle(
        "Fig. 5 — Chromatogram total estimated marginal means\n"
        "(z concentration & raw amount; note DAD Δ total mg·gDW⁻¹ classical p ≈ 0.0575).",
        fontsize=18,
    )
...
def figure07_freedman_lane() -> None:
    """Simple vs. Freedman–Lane permutation comparison."""
    simple_chrom = pd.read_csv(PROJECT_ROOT / "chromatogram_two_way_anova_permutation.csv")
    simple_chrom = simple_chrom[simple_chrom["term"] == "C(p_uva_mw_cm2):C(p_uvb_mw_cm2)"].copy()
    simple_chrom = simple_chrom[["dataset", "variant", "measurement", "p_perm"]]

    simple_dad = pd.read_csv(PROJECT_ROOT / "dad_two_way_anova_permutation.csv")
    simple_dad = simple_dad[simple_dad["term"] == "C(p_uva_mw_cm2):C(p_uvb_mw_cm2)"].copy()
    simple_dad = simple_dad[simple_dad["variant"] == "raw"]
    simple_dad = simple_dad[["dataset", "variant", "measurement", "p_perm"]]

    simple = pd.concat([simple_chrom, simple_dad], ignore_index=True)

    freedman = pd.read_csv(PROJECT_ROOT / "freedman_lane_interaction.csv")
    merged = simple.merge(
        freedman,
        on=["dataset", "variant", "measurement"],
        how="outer",
        validate="one_to_one",
    )
    merged["descriptor"] = (
        merged["dataset"] + " • " + merged["variant"] + " • " + merged["measurement"]
    )
    display = merged.melt(
        id_vars=["descriptor"],
        value_vars=["p_perm", "p_freedman_lane"],
        var_name="test",
        value_name="p_value",
    ...
    ax.set_title("Fig. 7 — Permutation tests for UVA×UVB interaction")
    ax.legend(
        title="Permutation method",
        labels=["Simple shuffle", "Freedman–Lane"],
        frameon=False,
        loc="lower right",
    )
    sns.despine(ax=ax)
...
def figure09_bootstrap() -> None:
    ...
    for ax, row, title in [
        (axes[0], beta_uvb, "β_UVB (Δ amount)"),
        (axes[1], beta_inter, "Interaction (Δ amount)"),
    ]:
        ax.axvline(0, color="black", linestyle="--")
        ax.errorbar(
            row["coef_mean"],
            0,
            xerr=[[row["coef_mean"] - row["coef_p2_5"]], [row["coef_p97_5"] - row["coef_mean"]]],
            fmt="o",
            color="#1f77b4",
            capsize=8,
            linewidth=3,
        )
        ax.set_xlim(min(row["coef_p2_5"], -0.5), max(row["coef_p97_5"], 2.8))
        ax.set_ylim(-1, 1)
        ax.set_yticks([])
        ax.set_title(title)
