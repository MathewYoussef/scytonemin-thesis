...
- Chromatogram raw metrics: no UVA, UVB, or interaction terms crossed α = 0.05 (p-values 0.099–0.297; `chromatogram_two_way_anova.csv`), suggesting no clear dose dependence in the unscaled data.
- Chromatogram normalized metrics: delta endpoints show UVA×UVB interactions with p ≈ 0.0085–0.013 and z-score endpoints with p ≈ 0.034–0.040 (η² ≈ 0.33–0.43); ratio and percent variants weaken to p ≈ 0.072–0.12. Continuous regressions highlight positive UVB coefficients for amount metrics (p ≈ 0.020–0.084) but with low R² (≤ 0.17).
